...
class PredictView(views.APIView):
	def post(self, request, endpoint_name, format=None):
		...
		if len(algs) != 1 and algorithm_status != "ab_testing":
			return Response(
				{"status": "Error", "message": "ML algorithm selection is ambiguous. Please specific algorithm version."},
				status = status.HTTP_400_BAD_REQUEST,
			)
	...
	def post(self, request, ab_test_id, format=None):
		...
		try:
			...
			correct_responses_1 = MLRequest.objects.filter(parent_mlalgorithm=ab_test.parent_mlalgorithm_1,
				created_at__gt=ab_test.created_at, created_at__lt=date_now, response=F('feedback')).count()

			if all_responses_1 > 0:
			...
			raise Response({"status": "Error", "message": str(e)}, status=status.HTTP_400_BAD_REQUEST)
